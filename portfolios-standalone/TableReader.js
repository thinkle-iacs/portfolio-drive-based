/* Source lives here:
 * https://github.com/thinkle/worfklows-jacknife/blob/master/TableReader.js
 */
...
/** @classdesc
 * Create a Table object representation of a range that can
...
function Table(range, idHeader) {
  ...
  function processRow(row) {
    ...
    function buildProperties(i, h) {
      // for closure purposes
      //logVerbose('Setting '+h+'->'+i);
      if (rowObj.hasOwnProperty(h)) {
        console.log(
          "Ignore duplicate column: %s (col %s). %s will refer to the first column by that name.",
          h,
          i,
          h
        );
      ...
      Object.defineProperty(rowObj, h, {
        enumerable: true,
        set: function (v) {
          var cell = sheet.getRange(
            Number(rowOffset) + Number(rowNum),
            Number(colOffset) + Number(i)
          );
          cell.setValue(v);
          row[i] = v;
        },
      ...
      Object.defineProperty(rowObj, i, {
        enumerable: true,
        set: function (v) {
          row[i] = v;
  ...
  var table = [];
  Object.defineProperty(table, "sheet", {
    value: sheet,
    configurable: false,
    writable: false,
  });
  Object.defineProperty(table, "range", {
    value: range,
    configurable: false,
    writable: false,
  });
